...
CURRENT PRIORITIES (in order): (1) the LIVE public site is stale and serves a wrong number — reconcile `main` for an SEO-safe deploy (`specs/deploy-reconcile.md`) and fix the real engine accuracy defect (`specs/accuracy-fix.md`); (2) the adversarial oracle / QA accuracy work (`specs/qa.md`); (3) everything else. SEO is crucial (the site is announced) but never at the cost of an accuracy regression. Parked / do-not-build: see `specs/plan.md`.
