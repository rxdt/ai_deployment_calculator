...
supported `weight_bits` value (16, 8, 4) and holding every other input constant.
...
- Input is `DeploymentSpec`; output is a typed comparison of the three supported
...
- For 8B / 8k / inference the comparison yields 16-bit=20.1, 8-bit=11.3, 4-bit=6.9
  with savings 0.0, 8.8, and 13.2 respectively.
...
- `build_report()` exposes the comparison; the web page shows all three totals.
