...
# Task: Improve the VRAM Deployment Calculator's spacing & layout

Single-page webapp at `frontend/` (Vite + vanilla TS, no framework). Improve the
calculator's **spacing/layout and visual polish**, grounded in three reference
calculators and the fitpick design system. Styling/colors come from fitpick;
layout rhythm comes from the references. Do NOT change calculation logic.

## Files you'll touch (and the only ones that matter for this)
- `frontend/src/styles.css` — all layout/spacing (≈335 lines, already on an 8px
  `--space-1..6` scale: 4/8/12/16/24/32). Grid: `main` is a 2-pane layout
  (`results` left, `controls` right 420px) + a top `terminal-bar`.
- `frontend/src/render.ts` — generates the markup as template strings:
  `renderStatusBar()`, `renderForm(state)` (the right-hand input panel),
  `renderResults(report, state)` (hero total + breakdown metric cards + hardware/
  quantization tables + assumptions).
- `frontend/src/app.ts` — assembles them into `#app`. Don't change behavior.

## Design system to pull from (colors, type, utilities)
- Path is `~/fitpick-ai/web/src/components` (NOT `~/fitpick/web/components`).
  Read `~/fitpick-ai/web/src/index.css` for tokens, and
  `components/InputForm.tsx` + `components/deployment/*` for spacing conventions.
- Tokens: `--primary #22c55e`, `--accent #4ade80`, `--green-dim #16a34a`,
  `--surface #0f0f14`, `--surface-raised #16161e`, `--card #111117`,
  `--border #27272a`, `--muted-foreground #71717a`, `--background #09090b`.
- Fonts: Geist Variable (sans) + JetBrains Mono (mono). Our calc is mono-only
  today — consider Geist for headings/values per `frontend/model_recommendation.png`.
- Reusable patterns: `hud-label` (mono, 0.65rem, letter-spacing .12em, uppercase),
  `hud-panel` (1px border + subtle top gradient over `--card`), `grid-bg`,
  `glow-green`. fitpick deployment components cluster on `gap-2`/`gap-3` (8/12px),
  `p-3` (12px). `frontend/model_recommendation.png` is the target aesthetic
  (terminal-green hero, dark grid bg).

## Reference spacing — ALREADY EXTRACTED (live computed CSS, don't re-fetch)
Sites + selectors (re-extract only if you want more detail; method at bottom):
- Odoo: https://design.odoo.com/calculator — `#x_wd_website_rogueone_calculator > form > div > div.col-xxl-8`
- Dispel: https://dispel.com/roi — `#main > div.framer-HhCIY.framer-lRnaX.framer-a66Wb.framer-1bvbs42`
- Groundcover: https://www.groundcover.com/calculator — `body > div.style-wrapper > main > section.p-section.calc1 > div.container.calc1-bottom > div.calc1-bottom-right`

| | Outer panel pad | Inner card pad | Card radius | Dominant gaps | Big # | Labels |
|---|---|---|---|---|---|---|
| Odoo | 40×32 | 16 | 10 / 20 / 36 | 8·16·24·32·40 | 24 | 14 |
| Dispel | 8 | 4×12 | 6 | 4·8·16 | large | 12–14 |
| Groundcover | 32×24 | 16 | 12 (999 pills) | 16·20·24·32 | 28 | 12–14 |
| Ours (current) | 24 | 12 | 6–8 | 8·12·16·24 | 56 | 11–13 |

**Takeaways:** all three are on an 8px base (our `--space-*` scale is correct).
Concrete deltas to apply: inner input/metric card padding should be **16px**
(`--space-4`), not 12px — every reference uses ~16px. Radius 6px matches Dispel;
Groundcover's 12px feels more modern if you want softer cards. Inter-group gap
20–24px is right. Confirm each value against the screenshots the user provided.

## Known issues the user reported (verify in-browser, then fix)
1. **A "Parameters (billions)" field looks duplicated.** Today `renderForm` emits
   both `Parameters (billions)` and `Active parameters (billions)`; on wide
   viewports they read as duplicates. Confirm there is no actual DOM duplication,
   then disambiguate labels/visual grouping (e.g. only show Active params for MoE,
   or group it under the Architecture field).
2. The user saw "many issues" beyond this — **start by running the app and doing a
   full visual pass at desktop (1440) and mobile (390)** and enumerate them before
   editing. Ask the user to confirm the list if unsure.

## Hard constraints (DO NOT break)
- Tests: `frontend/tests/calculator.spec.ts` pins `.total` class, the exact
  aria-labels (`Deployment inputs`, `Hardware recommendations`,
  `Quantization comparison`, `Assumptions`, `VRAM breakdown`, `Deployment status`),
  exact control accessible names (`getByLabel("Parameters (billions)", {exact:true})`,
  `select[name="weight_bits"]`, button `Calculate`, heading
  `VRAM Deployment Calculator`), and axe a11y (no violations). Any helper text MUST
  stay OUTSIDE `<label>` or you'll change accessible names and break tests.
- `npm run test` (vitest) enforces **100% coverage** (branches/functions/lines/
  statements). `npm run lint` runs a very strict eslint (`tseslint.configs.all` +
  unicorn/sonarjs/security) + stylelint + markuplint + cspell — keep it green.
- `frontend/harness/**` is a forbidden dir (gate drops agent edits). Don't touch it.

## How to run & observe (this works; localhost needs no network)
```bash
cd frontend && npm run dev      # serves http://127.0.0.1:5173/
```
Screenshot via the already-installed Playwright (write a tiny `.mjs` in `frontend/`
so `@playwright/test` resolves, screenshot 1440×900 and 390×844, then delete it).
To re-extract reference CSS, the same script pattern works but needs **network**
(run it outside the sandbox); use `waitUntil:"domcontentloaded"` + a 3s settle for
Groundcover (Webflow never reaches networkidle).

## Deliverable
Spacing/layout improvements in `styles.css` (+ minimal `render.ts` grouping changes
that preserve accessible names), validated against the reference table and the
user's screenshots, with `npm run test` + `npm run lint` green and a before/after
screenshot at both breakpoints. Fix the duplicated-field issue. Keep scope to
spacing/layout/visual polish — no tooling changes (zod/tsconfig/biome are out of scope).

### Original Task for the above - DO NOT REMOVE UNTIL OWNER CONFIRMS YOU RESEARCHED AND USED FINDINGS IN SPECS / CODE

![alt text](<Screenshot 2026-06-27 at 1.09.19 AM.png>)
![alt text](<Screenshot 2026-06-27 at 1.04.26 AM.png>)
![alt text](<Screenshot 2026-06-27 at 1.05.52 AM.png>)

Above are 3 examples of modern, simple calculators. They are not 1 page webapps but the eembedded caclulators could be. Use them for guidance on spacing and styling of inputs/outputs. I've pasted a screenshot, given the webiste link, and provide a link to an element selector for each calculator: use that to get the JS, CSS, and helpful element info by looking at the site. Here are links and selectors:

https://design.odoo.com/calculator -- #x_wd_website_rogueone_calculator > form > div > div.col-xxl-8

https://dispel.com/roi -- #main > div.framer-HhCIY.framer-lRnaX.framer-a66Wb.framer-1bvbs42

https://www.groundcover.com/calculator -- body > div.style-wrapper > main > section.p-section.calc1 > div.container.calc1-bottom > div.calc1-bottom-right

**How can you use tha tresearch to improve our current calculator? Especially around setting spacing on elements?** Turn research into actionable tasks.
